...
/**
 * Disjoint-set forest implementation of disjoint-set union, as given on page
 * 508 of <i>Introduction to Algorithms</i>, Second edition.
 */
...
public class DisjointSetForest implements DisjointSetUnion {
	...
	/**
	 * Private inner class to serve as opaque handles. Each node has a reference
	 * to its object. It also has a reference to its parent node and the value
	 * of its rank, made public so that they are accessible to the methods of
	...
	/**
	 * Links together two sets, given their root nodes. The root with the larger
	 * rank becomes the parent of the root with the smaller rank. In case of a
	 * tie, we arbitrarily choose one root as the parent of the other.
	 * 
	 * @param x
	 *            The root node of one set.
	 * @param y
